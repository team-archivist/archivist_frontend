...
export default class CommonUtils {
  ...
  public static typeOf = (o: unknown): string => {
    return (
      Object.prototype.toString.call(o).match(/\s([a-zA-Z]+)/) as string[]
    )[1].toLowerCase();
  };
  ...
  public static isFunction = <Fnc>(fn: unknown): fn is Fnc => {
    return (
      "function" === CommonUtils.typeOf(fn) ||
      "asyncfunction" === CommonUtils.typeOf(fn)
    );
  };
  ...
  public static getUserOS = (): { [key: string]: boolean } => {
    ...
    const userOS = Object.keys(OS).find(
      (os) => -1 !== navigator.appVersion.toLowerCase().indexOf(os)
    );
  ...
  public static async sequence(
    chains: { function: (data: unknown) => void; data: unknown }[],
    success = () => {},
    fallback = () => {}
  ) {
    /**
    ...
    async function waitNextBlock(
      chainData: { function: (data: unknown) => void; data: unknown },
      successCallback,
      fallbackCallback
    ) {
      try {
        ...
        await successCallback(
          !CommonUtils.isUndefined(chainData.data) ? chainData.data : {}
        );
      } catch (e) {
        fallbackCallback(
          !CommonUtils.isUndefined(chainData.data) ? chainData.data : {}
  ...
  public static array = <T>(
    collection: ArrayLike<T> | NodeList | HTMLCollection
  ): T[] => {
  ...
  public static isDomainString(str: string): boolean {
    return (
      /(\w+.(?:\.com|\.co\.kr|\.kr|\.org|\.net))/g.test(str) ||
      /^(\w+):(\/\/)?/.test(str)
    );
  }
  ...
  public static equals = (var1: unknown, var2: unknown): boolean => {
    const isVar1NonPrimitive =
      Array.isArray(var1) || CommonUtils.isObject(var1);

    const isVar2NonPrimitive =
      Array.isArray(var2) || CommonUtils.isObject(var2);
